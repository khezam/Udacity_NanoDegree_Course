...
"""One of the main point in this project to use a heap(min_proiority_queue) for faster insertion
    and deletion. Inserting is O(logn) and deletion is O(logn). Another important point is to use 
    admissible heuristic function h(x)<= cost_to_goal
"""

def insert(heap, node, element):
    """Insert nodes into the end of the heap and then calling siftUp() function to maintain a min-heap 
    """
    n = len(heap)
    if n == 0:
...
def euclidean(start, goal):
    """Calculating the distance between two points which is the estimated cost
    """
...
def shortest_path(start_node, end_node):
    ...
    visited = {}
    # path is the shortest distance between two nodes
    path = {start_node: None}
    # path_to_node is the shortes path from start to the final location
    ...
    while len(visited) < len(G):
        ...
        while i < len(node):
            if node[i] not in visited:
                # heuristics_dist is the current distance so far + the estimated cost to the goal(next node)
                heuristic_dist = current_score + euclidean(current_node, node[i])
                if node[i] not in dist_so_far:
                    # If the vertix does not exit in the heap then put it in there
                    ...
                    # If the vertix in the heap then check total cost if greater the heuristic_dist
                        # If it is, set the total cost to be heuristic_dist and the vertix.
